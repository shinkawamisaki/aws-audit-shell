...
aws iam get-account-authorization-details --filter User Group --output json | jq -r '
  ...
  | "| \($u.UserName) | \(join_or($ugs; "-")) | \(join_or($grp_managed|unique; "-")) | \(join_or($grp_inline|unique; "-")) | \(join_or(($u.AttachedManagedPolicies // [] | map(.PolicyName)); "❌")) |"
'

# 実行ログに追記（進捗表更新用）
echo "$(date +%F) | A02 実行完了" >> evidence_execution_log.md